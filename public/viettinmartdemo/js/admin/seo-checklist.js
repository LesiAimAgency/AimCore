window.SEOCheck = (function () {
    ...
    function run(id) {
        ...
        var allChecks = {
            ...
            seo_check_canonical:        { label: 'Liên kết Canonical đã được thiết lập đúng.', pass: (function() {
                var canEl = document.querySelector('[name="canonical_url"]');
                if (!canEl || !canEl.value.trim()) return false;
                try {
                    var url = new URL(canEl.value.trim());
    ...
    function init(id) {
        ['title', 'name', 'slug', 'meta_title', 'meta_description', 'canonical_url'].forEach(function (n) {
