...
@if($layout === 'slider')
    <!-- Widget #{{ uniqid() }}: Banner Slider (Style 1) -->
                            ...
                            <div class="swiper-wrapper">
                                ...
                                <div class="swiper-slide">
                                    <div class="banner-bg-image bg_image {{ $item['bg_class'] ?? 'bg_one-banner' }} ptb--120 ptb_md--80 ptb_sm--60"
                                         @if(!empty($item['image'])) style="background-image: url('{{ media_url($item['image']) }}')" @endif>
                ...
                <div class="col-lg-12">
                    <div class="banner-area-two-start">
                        <div class="banner-bg-iamge-area bg_image {{ $item['bg_class'] ?? 'bg_banner-2' }}" 
                             style="@if(!empty($item['image'])) background-image: url('{{ media_url($item['image']) }}') @endif">
                            <div class="content">
                                ...
                                <div class="rts-btn-banner-area">
                                    ...
                                    <div class="price-area">
                                        <span>{{ __('frontend.widget_banner_from') }}</span>
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <div class="banner-content">
                                @if(!empty($item['subtitle']))
                                    <span class="pre-title mb--15 d-block">{{ $item['subtitle'] }}</span>
                                @endif
                                <h1 class="title mb--30">{!! nl2br(e($item['title'] ?? '')) !!}</h1>
                                @if(!empty($item['btn_text']))
                                    <a href="{{ $item['btn_link'] ?? '#' }}" class="rts-btn btn-primary radious-sm">
                                        {{ $item['btn_text'] }} <i class="fa-light fa-arrow-right ml--10"></i>
                                    </a>
                                @endif
                            </div>
                        </div>
...
@elseif($layout === 'grid')
    ...
    @endphp
    <!-- Widget #{{ uniqid() }}: Promo Banners Grid -->
    <div class="{{ $outerClass }}" {!! $sectionStyles ?? '' !!}>
        <div class="{{ $containerClass }}">
            <div class="{{ $rowClass }} g-4">
                @foreach($slides as $item)
                <div class="{{ $gridClass }}">
                    <div class="single-feature-card bg_image {{ $item['bg_class'] ?? 'one' }}"
                         @if(!empty($item['image'])) style="background-image: url('{{ media_url($item['image']) }}'); background-size: cover; background-position: center; background-repeat: no-repeat;" @endif>
                        <div class="content-area">
                            ...
                            <a class="shop-now-goshop-btn" href="{{ $item['btn_link'] ?? '#' }}">
                                <span class="text">{{ $item['btn_text'] ?? __('common.shop_now') }}</span>
...
@else
    <!-- Widget #{{ uniqid() }}: Single Banner -->
    <div class="{{ $outerClass }}" {!! $sectionStyles ?? '' !!}>
        <div class="{{ $containerClass }}">
            @foreach($slides as $item)
            <div class="banner-area-two-start">
                <div class="banner-bg-iamge-area bg_banner-2 bg_image" @if(!empty($item['image'])) style="background-image: url('{{ media_url($item['image']) }}')" @endif>
                    <div class="content">
                        @if(!empty($item['subtitle'])) <span class="pre-title">{{ $item['subtitle'] }}</span> @endif
                        <h1 class="{{ $titleClass }}">{!! nl2br(e($item['title'] ?? '')) !!}</h1>
                        <div class="rts-btn-banner-area">
