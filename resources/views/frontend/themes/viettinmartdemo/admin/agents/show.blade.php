...
@section('title', 'Đại lý: ' . $agent->name)
@section('page-title', $agent->name)
@section('page-subtitle', $agent->type_name . ($agent->region ? ' · ' . $agent->region : ''))

@section('page-actions')
<div style="display:flex; gap:8px;">
    <a href="{{ locale_route('admin.agents.edit', $agent) }}" class="btn btn-primary">
...
<div style="display:grid;grid-template-columns:320px 1fr;gap:24px;align-items:start;">
    ...
    <div style="display:flex;flex-direction:column;gap:24px;">
        <div class="card" style="padding:32px 24px; text-align:center; border:none; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); border-radius:16px;">
            <div style="width:72px;height:72px;border-radius:20px;background:linear-gradient(135deg,#6366f1,#8b5cf6);display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:700;color:#fff;margin:0 auto 16px;box-shadow: 0 8px 16px rgba(99, 102, 241, 0.2);">
                {{ strtoupper(mb_substr($agent->name, 0, 1)) }}
            </div>
            <h2 style="font-size:18px;font-weight:800;color:#0f172a;margin-bottom:4px;">{{ $agent->name }}</h2>
            @if($agent->code)
                <span style="font-size:11px;background:#e0f2fe;color:#0369a1;padding:3px 12px;border-radius:20px;font-weight:800;display:inline-block;text-transform:uppercase;margin-bottom:12px;">{{ $agent->code }}</span>
            @endif
            <div>
                <span class="badge badge-orange" style="border-radius:20px; padding:4px 12px;">{{ $agent->type_name }}</span>
                <span class="badge {{ $agent->is_active ? 'badge-green' : 'badge-gray' }}" style="border-radius:20px; padding:4px 12px; margin-left:4px;">
                    {{ $agent->is_active ? 'Đang hoạt động' : 'Ngừng hoạt động' }}
                </span>
            </div>
            ...
            <div style="margin-top:24px; display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                <div style="background:#f8fafc; border-radius:12px; padding:12px;">
                    <p style="font-size:22px; font-weight:800; color:#0f172a;">{{ $orders->total() }}</p>
                ...
                <div style="background:#f0fdf4; border-radius:12px; padding:12px;">
                    <p style="font-size:16px; font-weight:800; color:#16a34a; line-height:22px;">{{ number_format($totalRevenue, 0, ',', '.') }}₫</p>
                    <p style="font-size:10px; color:#15803d; text-transform:uppercase; font-weight:700; letter-spacing:0.05em;">Doanh thu</p>
                </div>
            </div>
        </div>

        {{-- NEW: Tài khoản đăng nhập --}}
        <div class="card" style="border:none; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); border-radius:16px;">
            <div class="card-header" style="padding:16px 20px; background:#fcfcfc; border-bottom:1px solid #f1f5f9; border-radius:16px 16px 0 0;">
                <span class="card-title" style="font-size:14px; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;">Tài khoản hệ thống</span>
            </div>
                ...
                @if($agent->user)
                    <div style="display:flex; align-items:center; gap:12px; margin-bottom:12px;">
                        <div style="width:40px; height:40px; border-radius:50%; background:#e2e8f0; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                            <i class="fa-solid fa-user-circle" style="font-size:24px; color:#94a3b8;"></i>
                        </div>
                        <div style="overflow:hidden;">
                            <p style="font-size:14px; font-weight:700; color:#1e293b; margin-bottom:0; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;">{{ $agent->user->name }}</p>
                            <p style="font-size:12px; color:#64748b; margin-bottom:0; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;">{{ $agent->user->email }}</p>
                        </div>
                    </div>
                    <div style="display:grid; grid-template-columns:1fr; gap:8px;">
                        <span style="font-size:11px; color:#64748b;">Vai trò: <strong class="text-primary">{{ $agent->user->role ?: 'Đại lý' }}</strong></span>
                        <span style="font-size:11px; color:#64748b;">Trạng thái: <span class="badge badge-green" style="font-size:9px;">Hoạt động</span></span>
                    </div>
                @else
                    <div style="text-align:center; padding:10px 0;">
                        <p style="font-size:13px; color:#94a3b8; margin-bottom:12px;">Chưa liên kết tài khoản đăng nhập</p>
                        <a href="{{ locale_route('admin.agents.edit', $agent) }}" class="btn btn-secondary btn-sm w-100" style="font-size:12px; justify-content:center;">
                            <i class="fa-solid fa-link"></i> Liên kết ngay
                        </a>
                    </div>
                @endif
            </div>
            ...
            <div class="card-body" style="padding:20px; display:flex; flex-direction:column; gap:16px;">
                ...
                <div style="display:flex;align-items:center;gap:12px;">
                    <div style="width:32px;height:32px;border-radius:10px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="fa-solid fa-user" style="font-size:12px;color:#64748b;"></i>
                    </div>
                    <div>
                        <p style="font-size:10px;color:#94a3b8;text-transform:uppercase;font-weight:700;">Người liên hệ</p>
                        <p style="font-size:14px;font-weight:600;color:#334155;">{{ $agent->contact_person }}</p>
                    </div>
                </div>
                @endif
                @if($agent->phone)
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        ...
                        <a href="tel:{{ $agent->phone }}" style="font-size:14px;font-weight:700;color:#0f172a;text-decoration:none;">{{ $agent->phone }}</a>
                    </div>
                </div>
                @endif
                @if($agent->email)
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        ...
                        <a href="mailto:{{ $agent->email }}" style="font-size:14px;font-weight:700;color:#0f172a;text-decoration:none;">{{ $agent->email }}</a>
                    </div>
                </div>
                @endif
                @if($agent->region)
                <div style="display:flex;align-items:center;gap:12px;">
                    <div style="width:32px;height:32px;border-radius:10px;background:#fef9c3;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="fa-solid fa-map-location-dot" style="font-size:12px;color:#a16207;"></i>
                    </div>
                    <div>
                        <p style="font-size:10px;color:#94a3b8;text-transform:uppercase;font-weight:700;">Khu vực</p>
                        <p style="font-size:14px;font-weight:600;color:#334155;">{{ $agent->region }}</p>
                    </div>
                </div>
                @endif
                @if($agent->address)
                <div style="display:flex;align-items:flex-start;gap:12px;">
                    <div style="width:32px;height:32px;border-radius:10px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;">
                        <i class="fa-solid fa-location-dot" style="font-size:12px;color:#64748b;"></i>
                    </div>
                    <div>
                        <p style="font-size:10px;color:#94a3b8;text-transform:uppercase;font-weight:700;">Địa chỉ</p>
                        <p style="font-size:13px;color:#334155;line-height:1.6;">{{ $agent->address }}</p>
                    </div>
                </div>
                @endif
            </div>
        ...
        <div class="card" style="border:none; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); border-radius:16px;">
            <div class="card-header" style="padding:16px 20px; background:#fcfcfc; border-bottom:1px solid #f1f5f9; border-radius:16px 16px 0 0;">
                <span class="card-title" style="font-size:14px; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;">Ghi chú nội bộ</span>
            </div>
            <div class="card-body" style="padding:20px;">
                <p style="font-size:13px;color:#334155;line-height:1.6;white-space:pre-line;">{{ $agent->notes }}</p>
            </div>
        </div>
        @endif
    </div>
            ...
            <div style="padding:20px 24px; border-bottom:1px solid #f1f5f9; background:#fcfcfc; border-radius:16px 16px 0 0;">
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
                    <h3 style="font-size:12px; font-weight:800; color:#1e293b; text-transform:uppercase; letter-spacing:0.08em; display:flex; align-items:center; gap:10px;">
                        <span style="width:4px; height:16px; background:#6366f1; border-radius:4px; display:inline-block;"></span>
                        Lịch sử đơn hàng
                    </h3>
                ...
                <form action="{{ locale_route('admin.agents.show', $agent) }}" method="GET" style="display:flex; flex-wrap:wrap; gap:12px; align-items:flex-end;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:6px;">
                        ...
                        <select name="payment_status" class="form-select" style="width:160px; padding:6px 12px; border-radius:8px; height:36px; font-size:13px;">
                            <option value="">Tất cả</option>
                            @foreach($paymentStatuses as $key => $ps)
                    ...
                    @if(request()->anyFilled(['date_from', 'date_to', 'status', 'payment_status']))
                        <a href="{{ locale_route('admin.agents.show', $agent) }}" class="btn btn-secondary btn-sm" style="height:36px; padding:0 12px; border-radius:8px; font-weight:700;">
                            ...
                            <tbody>
                                ...
                                <tr class="tbl-tr">
                                    ...
                                    <td class="tbl-td" style="text-align:center; padding:14px 20px;">
                                        @php $sc = $statuses[$order->status] ?? ['label'=>$order->status,'color'=>'gray']; @endphp
                                        <span class="badge badge-{{ $sc['color'] }}" style="font-size:11px; border-radius:20px; padding:3px 12px; font-weight:700;">{{ $sc['label'] }}</span>
                                    </td>
                                    <td class="tbl-td" style="text-align:center; padding:14px 20px;">
                                        @php $psc = $paymentStatuses[$order->payment_status] ?? ['label'=>$order->payment_status,'color'=>'gray']; @endphp
