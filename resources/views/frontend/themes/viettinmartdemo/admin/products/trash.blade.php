...
@section('page-actions')
    <a href="{{ locale_route('admin.products.index') }}" class="btn btn-outline-secondary">
        ...
        <div class="card-header bg-white border-b border-slate-100 flex items-center justify-between py-5 px-6">
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-2">
                    {{-- Tab All --}}
                    <a href="{{ locale_route('admin.products.index') }}" title="Đang bán" class="relative w-12 h-12 flex items-center justify-center rounded-2xl transition-all duration-300 {{ !request()->routeIs('*.trash') ? 'bg-blue-600 shadow-xl shadow-blue-500/30' : 'bg-white border border-slate-100 shadow-sm' }}">
                    ...
                    {{-- Tab Trash --}}
                    <a href="{{ locale_route('admin.products.trash') }}" title="Thùng rác" class="relative w-12 h-12 flex items-center justify-center rounded-2xl transition-all duration-300 {{ request()->routeIs('*.trash') ? 'bg-blue-600 shadow-xl shadow-blue-500/30' : 'bg-white border border-slate-100 shadow-sm' }}">
                        ...
                        <tr class="hover:bg-rose-50/30 transition-all group">
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <form action="{{ locale_route('admin.products.restore', $product->id) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    <form action="{{ locale_route('admin.products.force-delete', $product->id) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
