    ...
    <div>
        ...
        <ul style="margin:4px 0 0 16px;padding:0;">
            @foreach($errors->all() as $e)
                <li style="font-size:12.5px;">{{ $e }}</li>
            @endforeach
        </ul>
    </div>
</div>
...
<script>
    ...
    const maxContainer = document.getElementById('max_discount_container');
    ...
    function applyTypeUI(type) {
        if (type === 'percentage') {
            ...
            // value field: chỉ nhập số 0-100, không format money
            valueInput.classList.remove('money-input');
            valueInput.placeholder = 'VD: 20 (%)';
            valueInput.max = 100;
        } else {
            maxContainer.classList.add('hidden');
            // value field: format money
            valueInput.classList.add('money-input');
            valueInput.placeholder = '';
            valueInput.removeAttribute('max');
            valueInput.value = formatMoney(valueInput.value);
        }
    }
    ...
    // Money Input Formatter — chỉ áp dụng cho .money-input
