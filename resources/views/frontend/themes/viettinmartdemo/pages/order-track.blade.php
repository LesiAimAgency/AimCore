...
@section('title', Lang('track_order_title'))
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="navigator-breadcrumb-wrapper">
                    ...
                    <a class="current" href="#">{{ Lang('track_order_title') }}</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row justify-content-center">
            <div class="col-xl-8 col-lg-10">
                <div class="tracing-order-account">
                    <h2 class="title text-center mb--15" style="font-size: 32px; font-weight: 700;">{{ Lang('track_order_title') }}</h2>
                    <p class="text-center mb--40 text-muted mx-auto" style="max-width: 700px; font-size: 16px; line-height: 1.6;">
                        {{ Lang('track_order_description') }}
                    </p>
                    ...
                    <div class="tracking-form-card p--40 mb--50 shadow-sm border-radius-12 bg-white border border-light">
                        <form action="{{ locale_route('order.track') }}" method="GET" class="order-tracking">
                            <div class="row g-4 d-flex align-items-center">
                                <div class="col-md-5">
                                    <div class="single-input">
                                        <label for="order-id" class="mb--10 font-bold text-heading small opacity-75">{{ strtoupper(Lang('order_number')) }}</label>
                                ...
                                <div class="col-md-5">
                                    <div class="single-input">
                                        <label for="order-idt" class="mb--10 font-bold text-heading small opacity-75">{{ strtoupper(Lang('billing_email')) }}</label>
                                        <div class="input-wrapper position-relative">
                        ...
                        <div class="order-tracking-result bg-white border-radius-12 shadow-lg overflow-hidden animate__animated animate__fadeInUp">
                            ...
                            <div class="result-header p--40 text-white d-flex justify-content-between align-items-center flex-wrap gap-4" style="background: linear-gradient(135deg, var(--color-primary) 0%, #4a7d1a 100%); min-height: 120px;">
                                <div class="header-content">
                                    <span class="text-uppercase small fw-bold opacity-75 mb-2 d-block" style="font-size: 12px; letter-spacing: 1.5px;">{{ Lang('order_status') }}</span>
                            ...
                            <div class="tracking-stepper p--50 border-bottom bg-white">
                                @php
                                    $steps = [
                                        'pending' => ['icon' => 'fa-file-alt', 'label' => Lang('order_status_pending')],
                                        'confirmed' => ['icon' => 'fa-check-circle', 'label' => Lang('order_status_confirmed')],
                                        'processing' => ['icon' => 'fa-box', 'label' => Lang('order_status_processing')],
                                        'shipping' => ['icon' => 'fa-shipping-fast', 'label' => Lang('order_status_shipping')],
                                        'delivered' => ['icon' => 'fa-home', 'label' => Lang('order_status_delivered')]
                                    ];
                                ...
                                <div class="row g-5">
                                    ...
                                    <div class="col-lg-7">
                                        <h6 class="mb--30 section-title fw-bold text-uppercase d-flex align-items-center">
                                            <i class="fa-light fa-list-check me-2 text-primary"></i> {{ Lang('product_list') }}
                                        </h6>
                                    ...
                                    <div class="col-lg-5">
                                        ...
                                        <div class="info-card p-4 border-radius-10 bg-light-soft mb-4 border border-faint-1 shadow-sm">
                                            <h6 class="mb-4 text-uppercase small text-muted font-bold d-flex align-items-center" style="font-size: 12px; letter-spacing: 1px;">
                                                <i class="fa-light fa-truck me-2"></i> {{ Lang('shipping_payment_info') }}
                                            </h6>
                                            ...
                                            <div class="d-flex gap-3">
                                                ...
                                                <div>
                                                    <p class="mb-1 text-heading fw-bold small">{{ Lang('payment_method_status') }}</p>
                                                    <p class="mb-1 text-muted small">{{ Lang('payment_gateway') }}: <span class="text-heading fw-bold">{{ strtoupper($order->payment_method) }}</span></p>
                                                    <p class="mb-0 text-muted small">{{ Lang('payment_status') }}: <span class="text-{{ $order->payment_status == 'paid' ? 'success' : 'danger' }} fw-bold">{{ $order->payment_status_label }}</span></p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="summary-card p-4 border-radius-10 bg_primary-light border-0 shadow-sm">
                                            <h6 class="mb-4 text-uppercase small text-primary font-bold text-center" style="font-size: 12px; letter-spacing: 1px;">{{ Lang('order_summary') }}</h6>
                                            <div class="d-flex justify-content-between mb-3 text-dark small">
                                                <span class="opacity-70 fw-medium">{{ Lang('subtotal') }} ({{ count($order->items) }} {{ Lang('items') }})</span>
                                                <span class="fw-bold">{{ number_format($order->subtotal, 0, ',', '.') }}đ</span>
                                            </div>
                                            <div class="d-flex justify-content-between mb-3 text-dark small">
                                                <span class="opacity-70 fw-medium">{{ Lang('shipping_fee') }}</span>
                                            ...
                                            @if($order->discount > 0)
                                                <div class="d-flex justify-content-between mb-3 text-danger small">
                                                    <span class="opacity-70 fw-medium">{{ Lang('discount_applied') }}</span>
                                            ...
                                            <div class="mt-4 pt-3 border-top border-primary border-opacity-10 d-flex justify-content-between align-items-end">
                                                <h6 class="mb-0 fw-bold text-dark">{{ strtoupper(Lang('total')) }}</h6>
                            ...
                            <div class="p--30 bg-light-soft text-center border-top">
                                <p class="mb-0 text-muted small">{{ Lang('contact_support') }}: <a href="tel:{{ setting('hotline', '02345697871') }}" class="text-primary font-bold ms-1" style="font-size: 15px;">{{ setting('hotline', '02345697871') }}</a></p>
                            </div>
                        </div>
                    @endif
                </div>
