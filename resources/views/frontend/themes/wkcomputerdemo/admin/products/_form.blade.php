@php
    ...
    if ($isEdit && $hasI18n) {
        foreach ($otherLangs as $lang) {
            foreach (['name','short_description','description','additional_info','meta_title','meta_description','promotions'] as $f) {
    ...
    $getImgUrl = function($img) {
        if (!$img) return asset('admin/images/placeholder.webp');
        if (str_starts_with($img, 'http')) return $img;
        $path = ltrim($img, '/');
        if (str_starts_with($path, 'media/')) return \Storage::disk('public')->url($path);
        return asset($path);
    };
    ...
    $comboInitData = [];
    if ($isEdit) {
        $comboInitData = $product->combos->map(function($c) use ($getImgUrl) {
    ...
    // Prepare flattened list for the visual picker
    $searchableProducts = [];
    foreach ($allProducts ?? [] as $p) {
        ...
        if ($p->has_variants && $p->variants->count() > 0) {
            foreach ($p->variants as $v) {
                $searchableProducts[] = [
                    'key' => $p->id . ':' . $v->id,
                    'name' => $p->name . ' (' . ($v->label ?: ($v->sku ?: 'v#' . $v->id)) . ')',
                    'price' => (float)($v->price ?: $p->price),
                    'image' => $getImgUrl($v->image ?: $p->image)
                ];
            }
        } else {
            $searchableProducts[] = [
...
window.productFormManager = function(existingVariants, existingAttrMap) {
    return {
        ...
        comboSearch: '',
        allSearchProducts: @json($searchableProducts),
        combos: @json($comboInitData ?? []),

        get filteredSearchProducts() {
            if (!this.comboSearch) return this.allSearchProducts.slice(0, 40);
            const q = this.comboSearch.toLowerCase();
            return this.allSearchProducts.filter(p => p.name.toLowerCase().includes(q)).slice(0, 100);
        },

        toggleBatchSelection(key) {
            if (this.batchSelectedProducts.includes(key)) {
                this.batchSelectedProducts = this.batchSelectedProducts.filter(k => k !== key);
            } else {
                this.batchSelectedProducts.push(key);
            }
        },

        toggleAllVisible() {
            const visibleKeys = this.filteredSearchProducts.map(p => p.key);
            const allSelected = visibleKeys.every(k => this.batchSelectedProducts.includes(k));
            if (allSelected) {
                this.batchSelectedProducts = this.batchSelectedProducts.filter(k => !visibleKeys.includes(k));
            } else {
                visibleKeys.forEach(k => {
                    if (!this.batchSelectedProducts.includes(k)) this.batchSelectedProducts.push(k);
                });
            }
        },
        ...
        get isAllVisibleSelected() {
            const visibleKeys = this.filteredSearchProducts.map(p => p.key);
            return visibleKeys.length > 0 && visibleKeys.every(k => this.batchSelectedProducts.includes(k));
        },
        ...
        addCombo(pIdAndVariantId, discountVal = null, discountType = null) {
            ...
            const fixImg = (img) => {
                if (!img) return '/admin/images/placeholder.webp';
                if (img.startsWith('http')) return img;
                if (img.startsWith('/')) return img;
                if (img.startsWith('media/')) return '/storage/' + img;
                return '/' + img;
            };
    ...
    <div class="space-y-6">
        ...
        <div class="card shadow-sm border-slate-200">
            ...
            <div class="card-body space-y-4">
                @if($hasI18n)
                <div x-data="{ 
                    activeTab: '{{ $defaultLang?->code ?? 'vi' }}',
                    copyFromDefault(targetLang) {
                        // Copy giá trị từ tab mặc định sang tab đang xem
                        const fields = ['name', 'short_description', 'description', 'additional_info', 'promotions'];
                        fields.forEach(field => {
                            const src = document.querySelector(`[name='${field}']`);
                            const dst = document.querySelector(`[name='translations[${targetLang}][${field}]']`);
                            if (src && dst) {
                                dst.value = src.value;
                                // Trigger TinyMCE nếu có
                                if (window.tinymce) {
                                    const srcEd = tinymce.get(src.id);
                                    const dstEd = tinymce.get(dst.id);
                                    if (srcEd && dstEd) dstEd.setContent(srcEd.getContent());
                                }
                            }
                        });
                    }
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 mb-4">
                        <div class="flex gap-0 flex-wrap">
                            ...
                            <button type="button"
                                ...
                                {{ $lang->name }}
                                @if($lang->is_default)
                                    <span class="text-[9px] bg-blue-100 text-blue-500 px-1.5 py-0.5 rounded font-black uppercase">Mặc định</span>
                                @elseif($isEdit)
                                    @php
                                        $hasName = $product->translations->where('locale', $lang->code)->where('field', 'name')->whereNotNull('value')->where('value', '!=', '')->isNotEmpty();
                                    @endphp
                                    <span class="text-[9px] px-1.5 py-0.5 rounded font-black uppercase {{ $hasName ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600' }}">
                                        {{ $hasName ? 'Đã dịch' : 'Chưa dịch' }}
                                    </span>
                                @endif
                            </button>
                            @endforeach
                        </div>
                        {{-- Nút copy từ ngôn ngữ mặc định --}}
                        @foreach($otherLangs as $lang)
                        <button type="button"
                                x-show="activeTab === '{{ $lang->code }}'"
                                @click="copyFromDefault('{{ $lang->code }}')"
                                class="text-[10px] font-black text-slate-400 hover:text-blue-600 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 hover:border-blue-300 transition-all mb-1">
                            <i class="fa-solid fa-copy"></i> Copy từ {{ $defaultLang?->name }}
                        </button>
                        @endforeach
                    </div>
        ...
        <div class="card shadow-sm border-slate-200 overflow-hidden">
            ...
            <div class="flex min-h-[400px]">
                ...
                <div class="flex-1 p-5 bg-white overflow-hidden">
                                ...
                                <div class="space-y-5">
                                    <div class="w-full space-y-4">
                                        <div class="flex items-center justify-between">
                                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">1. Chọn sản phẩm mua kèm (Sản phẩm B)</label>
                                            <div class="flex items-center gap-2">
                                                <span class="text-[10px] font-black text-blue-500 uppercase" x-text="batchSelectedProducts.length + ' đã chọn'"></span>
                                                <button type="button" @click="batchSelectedProducts = []" x-show="batchSelectedProducts.length > 0" class="text-[10px] font-bold text-rose-500 hover:underline">Bỏ chọn hết</button>
                                            </div>
                                        </div>
                                        
                                        <div class="relative">
                                            <i class="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                                            <input type="text" x-model="comboSearch" placeholder="Gõ tên sản phẩm để tìm nhanh..." 
                                                   class="form-input !pl-11 !py-3 !text-sm border-slate-200 rounded-2xl shadow-sm focus:ring-blue-100 transition-all">
                                        </div>

                                        <div class="flex items-center justify-between px-2">
                                            <label class="flex items-center gap-2 cursor-pointer">
                                                <input type="checkbox" @click="toggleAllVisible()" :checked="isAllVisibleSelected" 
                                                       class="w-4.5 h-4.5 rounded border-slate-300 text-blue-600 transition-all">
                                                <span class="text-[10px] font-black text-blue-600 uppercase tracking-widest">Chọn tất cả</span>
                                            </label>
                                            <span class="text-[9px] font-bold text-slate-400 uppercase italic">Hiển thị <span x-text="filteredSearchProducts.length"></span> kết quả</span>
                                        </div>

                                        <div class="combo-list flex flex-col gap-2 max-h-[280px] overflow-y-auto p-2 custom-scroll bg-slate-50/50 rounded-2xl border border-slate-100">
                                            <template x-for="p in filteredSearchProducts" :key="p.key">
                                                <div @click="toggleBatchSelection(p.key)"
                                                     :class="batchSelectedProducts.includes(p.key) ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-500/10' : 'border-white hover:border-slate-200 bg-white'"
                                                     class="flex items-center gap-3 p-2.5 rounded-xl border-2 shadow-sm cursor-pointer transition-all group">
                                                    
                                                    <div class="relative">
                                                        <img :src="p.image" class="w-12 h-12 rounded-lg object-cover shadow-sm group-hover:scale-105 transition-transform">
                                                        <div x-show="batchSelectedProducts.includes(p.key)" 
                                                             class="absolute -top-1.5 -right-1.5 w-5 h-5 bg-blue-600 text-white rounded-lg flex items-center justify-center shadow-lg border-2 border-white scale-110">
                                                            <i class="fa-solid fa-check text-[9px]"></i>
                                                        </div>
                                                    </div>
                                                    
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-[11px] font-black text-slate-700 uppercase truncate leading-tight" x-text="p.name"></p>
                                                        <p class="text-[10px] font-bold text-blue-500 mt-1" x-text="new Intl.NumberFormat('vi-VN').format(p.price) + '₫'"></p>
                                                    </div>

                                                    <input type="checkbox" :checked="batchSelectedProducts.includes(p.key)" 
                                                           class="w-5 h-5 rounded-lg border-slate-200 text-blue-600 cursor-pointer pointer-events-none">
                                                </div>
                                            </template>
                                            <template x-if="filteredSearchProducts.length === 0">
                                                <div class="py-10 text-center">
                                                    <i class="fa-solid fa-magnifying-glass text-slate-200 text-2xl mb-2"></i>
                                                    <p class="text-[10px] text-slate-400 font-bold uppercase">Không tìm thấy sản phẩm</p>
                                                </div>
                                            </template>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col lg:flex-row gap-4 items-end pt-5 border-t border-blue-100/50">
                                        ...
                                        <div class="flex-shrink-0">
                                            <button type="button" @click="addBatchCombos()" 
                                                    class="h-[38px] px-6 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-500/10 transition-all active:scale-95 flex items-center gap-2 whitespace-nowrap">
                                                <i class="fa-solid fa-plus font-bold"></i> THÊM
                                            </button>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="space-y-6">
        ...
        <div class="card shadow-sm border-slate-200 border-l-4 border-l-teal-400">
            <div class="card-body space-y-5">
                ...
                <x-admin.seo-checklist context="product" :model="$product" id="product_seo_checklist" />

                <hr class="border-slate-100 my-4">

                @if($hasI18n)
                {{-- SEO có tab ngôn ngữ cho meta_title và meta_description --}}
                <div x-data="{ seoTab: '{{ $defaultLang?->code ?? 'vi' }}' }">
                    <div class="flex gap-0 border-b border-slate-100 mb-4">
                        @foreach($languages as $lang)
                        <button type="button"
                                @click="seoTab = '{{ $lang->code }}'"
                                :class="seoTab === '{{ $lang->code }}' ? 'border-b-2 border-teal-500 text-teal-600 bg-teal-50' : 'text-slate-400 hover:text-slate-600'"
                                class="flex items-center gap-1 px-3 py-1.5 text-[10px] font-black rounded-t-lg transition-all -mb-px uppercase">
                            @if($lang->flag_emoji)<span>{{ $lang->flag_emoji }}</span>@endif
                    ...
                    {{-- Tab ngôn ngữ mặc định --}}
                    <div x-show="seoTab === '{{ $defaultLang?->code }}'" x-cloak class="space-y-4">
                        <div>
                    ...
                    {{-- Tab ngôn ngữ khác --}}
                    @foreach($otherLangs as $lang)
                    <div x-show="seoTab === '{{ $lang->code }}'" x-cloak class="space-y-4">
                        <div>
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Tiêu đề SEO ({{ strtoupper($lang->code) }})</label>
                            <input type="text" name="translations[{{ $lang->code }}][meta_title]"
                                   value="{{ old('translations.' . $lang->code . '.meta_title', $existingTranslations[$lang->code]['meta_title'] ?? '') }}"
                                   class="form-input !py-2 !text-xs bg-slate-50 border-none shadow-inner" placeholder="Để trống = dùng bản {{ strtoupper($defaultLang?->code) }}">
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Mô tả SEO ({{ strtoupper($lang->code) }})</label>
                            <textarea name="translations[{{ $lang->code }}][meta_description]" rows="3"
                                      class="form-input !py-2 !text-xs bg-slate-50 border-none shadow-inner resize-none"
                                      placeholder="Để trống = dùng bản {{ strtoupper($defaultLang?->code) }}">{{ old('translations.' . $lang->code . '.meta_description', $existingTranslations[$lang->code]['meta_description'] ?? '') }}</textarea>
                        </div>
                    </div>
                    @endforeach
                </div>
...
<script>
    $(document).ready(function() {
        // Scripts related to other components can go here
    });
</script>
<style>
