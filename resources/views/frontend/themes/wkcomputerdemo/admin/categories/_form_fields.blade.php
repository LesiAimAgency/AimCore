...
<div x-data="{ tab: 'basic' }" style="padding:16px 20px 0;">
    ...
    <div x-show="tab==='basic'" style="display:flex;flex-direction:column;gap:14px;">
        ...
        {{-- Categories cha (chỉ hiển thị cho danh mục bài viết) --}}
        <div x-data="{ type: '{{ $type ?? 'post' }}' }" x-show="type === 'post'">
    ...
    {{-- Tab: SEO --}}
    <div x-show="tab==='seo'" x-cloak style="display:flex;flex-direction:column;gap:14px;padding-bottom:10px;">
        
        <x-admin.seo-checklist context="category" :model="$cat" :id="$p . '_seo_checklist'" />
        ...
        <div>
            ...
            <input type="url" name="canonical_url" id="{{ $p }}_canonical_url"
                   class="form-input" placeholder="https://...">
            <p class="form-hint">Để trống để tự động lấy URL của danh mục này.</p>
        </div>
