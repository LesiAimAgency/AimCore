@php
    ...
    if ($isEdit && $hasI18n) {
        $existingTranslations = $post->getTranslationsArray(['title', 'content', 'excerpt', 'meta_title', 'meta_description']);
    }
@endphp
        ...
        <div class="card shadow-sm border-slate-200">
            <div class="card-header bg-slate-50/50">
                <p class="text-sm font-bold text-slate-800 flex items-center gap-2">
                    <i class="fa-solid fa-align-left text-blue-500"></i> Nội dung bài viết
                </p>
            </div>
            <div class="card-body">
                @if($hasI18n)
                <div x-data="{ activeTab: '{{ $defaultLang?->code ?? 'vi' }}' }">
                    ...
                    <div x-show="activeTab === '{{ $defaultLang?->code }}'" x-cloak>
                        @include('admin.posts._fields_basic', ['locale' => null, 'prefix' => ''])
                    </div>
                    @foreach($otherLangs as $lang)
                    <div x-show="activeTab === '{{ $lang->code }}'" x-cloak>
                        @include('admin.posts._fields_basic', [
                            'locale' => $lang->code,
