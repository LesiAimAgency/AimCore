...
<div class="category-feature-area rts-section-gapTop" {!! $sectionStyles ?? '' !!}>
    <div class="container">
        <div class="row g-4">
            ...
            @php
                $bgStyles = ['one', 'two', 'three', 'four'];
                $bgStyle = $item['bg_style'] ?? ($bgStyles[$index % 4] ?? 'one');
                $img = $item['image'] ?? null;
                $imgUrl = !empty($img) ? media_url($img) : null;
            @endphp
            <div class="{{ $gridClass }}">
                <div class="single-feature-card bg_image {{ $bgStyle }}"
                     @if($imgUrl) style="background-image: url('{{ $imgUrl }}'); background-size: cover; background-position: center; background-repeat: no-repeat;" @endif>
                    <div class="content-area">
                        ...
                        <a class="shop-now-goshop-btn" href="{{ $item['btn_link'] ?? '#' }}">
                            <span class="text">{{ $item['btn_text'] ?? __('frontend.product_buy_now') }}</span>
                            <div class="plus-icon">
