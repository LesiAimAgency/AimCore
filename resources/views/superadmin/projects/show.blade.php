  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <form method="POST" action="{{ route('superadmin.projects.deploy-vtm', $project) }}" class="inline-block" onsubmit="return confirm('Bạn có chắc chắn muốn Triển khai Mẫu Viettinmart (1-Click VTM)? Toàn bộ Theme, 21 Module, Menu, Widgets và Dữ liệu mẫu eCommerce sẽ được tự động cài đặt.')">
        @csrf
        <button type="submit" class="px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg hover:from-emerald-700 hover:to-teal-700 text-xs sm:text-sm font-semibold inline-flex items-center gap-1.5 shadow-xs transition-all">
    ...
    <div class="lg:col-span-2 space-y-4 sm:space-y-6">
      <div class="bg-white rounded-xl shadow-xs p-4 sm:p-6 border border-gray-100">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
          ...
          <div class="col-span-2 p-3.5 bg-gradient-to-br from-amber-50 to-yellow-50/80 rounded-xl border border-amber-200">
            <div class="flex items-center justify-between mb-2">
              <span class="text-sm font-bold text-amber-900 flex items-center gap-1.5">
                <svg class="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8" fill="#FBBF24"/><text x="10" y="13" font-size="8" font-weight="bold" fill="#78350F" text-anchor="middle">G</text></svg>
                Quỹ Điểm Thưởng Gold Dự Án
              </span>
              <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-amber-200 text-amber-900">
                Tổng: {{ number_format($project->total_gold ?? 0) }} Gold
              </span>
            </div>
            <div class="grid grid-cols-3 gap-2 text-xs pt-1 border-t border-amber-200/60">
              <div>
                <span class="text-gray-500 block">Tổng quỹ:</span>
                <span class="font-bold text-gray-900">{{ number_format($project->total_gold ?? 0) }} G</span>
              </div>
              <div>
                <span class="text-gray-500 block">Đã phân bổ:</span>
                <span class="font-bold text-amber-800">{{ number_format($project->allocatedGold()) }} G</span>
              </div>
              <div>
                <span class="text-gray-500 block">Còn khả dụng:</span>
                <span class="font-bold text-emerald-700">{{ number_format($project->remainingGold()) }} G</span>
              </div>
            </div>
          </div>
          @endif
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4 border-b pb-2">Thông tin triển khai & Ghi chú</h3>
        <div class="space-y-4">
          <div>
