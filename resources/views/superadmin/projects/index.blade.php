...
<div class="px-1 sm:px-6 py-3 sm:py-8 w-full max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
        <div>
            <h1 class="text-xl sm:text-3xl font-bold text-[#001B4E]">Danh sách Dự án</h1>
            <p class="text-gray-500 text-xs sm:text-sm mt-0.5 sm:mt-1">Quản lý và theo dõi tiến độ các dự án đang triển khai</p>
        </div>
        @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('manage-projects'))
        <div>
            <a href="{{ route('superadmin.projects.create') }}" class="w-full sm:w-auto justify-center px-4 sm:px-6 py-2 sm:py-2.5 bg-[#001B4E] text-white rounded-lg hover:bg-[#002D80] font-medium inline-flex items-center transition-colors shadow-xs text-xs sm:text-sm">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="py-4 px-6 text-sm font-semibold text-gray-600 uppercase tracking-wider">Quỹ Gold</th>
                        @endif
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        @if(config('features.gold_enabled'))
                        <td class="py-4 px-6">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold">
                                ...
                                {{ number_format($project->total_gold ?? 0) }} G
                            </span>
                        </td>
                        @endif
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form method="POST" action="{{ route('superadmin.projects.deploy-wkcomputer', $project) }}" class="inline-block" onsubmit="return confirm('Triển khai Mẫu WKComputer (1-Click WK) cho dự án {{ $project->name }}?');">
                                    @csrf
                                    <button type="submit" class="p-2 text-rose-600 bg-rose-50 rounded-lg hover:bg-rose-100 transition-colors" title="Deploy 1-Click WKComputer">
