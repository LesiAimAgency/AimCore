...
$users = \App\Models\User::withoutGlobalScopes()->get(['id', 'username', 'email', 'role', 'level', 'tenant_id', 'project_ids']);
foreach ($users as $u) {
    echo "ID: {$u->id}, User: {$u->username}, Role: {$u->role}, Tenant: {$u->tenant_id}, ProjectIds: " . json_encode($u->project_ids) . "\n";
}

$projects = \App\Models\Project::all(['id', 'code', 'name', 'tenant_id']);
echo "\n--- PROJECTS ---\n";
