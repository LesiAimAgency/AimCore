...
$user = User::where('username', 'viettinmart-eco')->orWhere('email', 'admin@viettinmart.vn')->first() ?? User::first();

echo 'Project Code: '.$project->code."\n";
echo 'Project Features Theme: '.($project->features['theme'] ?? 'none')."\n";
echo "Project Settings 'theme': ".ProjectSetting::where('project_id', $project->id)->where('key', 'theme')->value('value')."\n";
...
foreach ($urls as $url) {
    $request = Request::create($url, 'GET', [], [], [], ['REMOTE_ADDR' => '127.0.0.1']);
